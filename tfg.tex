...
\begin{defn}
  ...
  \[
    \pi W\to\mathbb Q
  \]
  such that \( W \) is a local ring with maximal ideal \( I=+p^{-1}(0) \) with \( I \) nilpotent and such that \( W \) is finite dimensional as a \( \mathbb{Q} \) vector space.
\end{defn}
...
\begin{lemma}
  ...
  \[
    \mathbb{Q}[X_1,\dots,X_r]/(P_1,\dots,P_s)
  \]
  where \( P_1,\dots P_s \) are polynomials of \( \mathbb{Q}[X_1,\dots,X_r] \).
  \label{lm:finpr}
\end{lemma}
...
\begin{proof}
  ...
  \]
  in other words, the kernel of the morphism
  \[
    R[X_1,\dots,X_n]\to W
  \]
