...
\begin{proof}
  ...
  \]
  Recall that the \( d_r \) are in \( D \), so only the summands with unit powers of each \( d_r \) will be non-zero. That is, choosing a subset of the \( i_r \) to be 1, and the rest 0. In that case the multinomial coefficients are simply \( k! \), giving us
  \[
    \delta^k = \sum_{I\subset \{1,\dots,n\}}k!\prod_{t\in I}d_t
  \]
  In other words, the \( k \)-\textit{th} elementary symmetric polynomial of \( n \) variables (multiplied by \( k! \)). Denote this by \( e_k \). We proceed inductively:
  \begin{align*}
    f(x+\delta) & = f(x+d_1+\cdots+d_n) \\
		...
		& = \sum_{i=0}^{n-1}f^{(i)}(x)\left( e_i(d_1,\dots,d_{n-1})+d_ke_{i-1}(d_1,\dots,d_{n-1})\right) + d_nf^{(n)}e_n(d_1,\dots,d_{n-1})
  \end{align*}
\end{proof}
