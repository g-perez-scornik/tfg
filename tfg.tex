...
\subsection{A closer look at the first axiom}
...
\begin{defn}
  ...
  \begin{equation*}
    \pi: W\to\mathbb Q
  \end{equation*}
  such that \( W \) is a local ring with maximal ideal \( I=\pi^{-1}(0) \) with \( I \) nilpotent and such that \( W \) is finite dimensional as a \( \mathbb{Q} \) vector space. A homomorphism of Weil algebras \( f:W_1\to W_2 \) is thus an algebra homomorphism sending the maximal ideal of \( W_1 \) to that of \( W_2 \).
\end{defn}
...
\begin{align*}
  W_1 &\cong \mathbb{Q}[X_1,\dots,X_n]/I \\
  W_2 &\cong \mathbb{Q}[Y_1,\dots,Y_n]/J
\end{align*}
...
So, by virtue of the functoriality we get as a corollary that isomorphic Weil algebras induce a bijection on their spectra.
...
Consider two functions \( f,g:D\to X \) such that \( f(0)=g(0) \), where \( X \) is some set (object of \(  \mathcal{E}) \). Here there is no reason to assume that this will define a map \( D(2)\to X \). It will be the case, however, if \( X \) is \( R \).
