...
That a Weil algebra is finite dimensional over \( \mathbb{Q} \) means that it is isomorphic to \( \mathbb{Q}^n \) with a certain product given by \( \gamma_{ij}^k \), for some \( n \). If \( A \) is any other \( \mathbb{Q} \)-algebra, then so can \( A^n \) be endowed with a product determined by the same constants.
